...
import { CategorySummary } from "../../types/category.type";
...
export const getTransactionsSummary = async( request: FastifyRequest<{Querystring: GetTransactionSummaryQuery}>, reply: FastifyReply): Promise< void> =>{
    ...
    if(!month || !year){
        return reply.status(400).send({error: "Month and year are required!"})
    }
    ...
    try {
       ...
       const summary: TransactionSummary = {
        ...
        expensesByCategory: Array.from(groupExpenses.values()).map((entry) => ({
            ...entry,
       ...
       reply.send(summary)
        ...
        reply.status(500).send({error:"An error occurred while fetching transactions."});
      }
