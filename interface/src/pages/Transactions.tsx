        ...
        <div className="container-app py-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                ...
                <Link to="/transacoes/nova"
                    className="bg-primary-500 text-[#051626] px-4 py-2.5 rounded-xl flex items-center justify-center hover:bg-primary-600 transition-all">
            ...
            <Card className="overflow-hidden">
                {loading ? (
                    ...
                    <div className="text-center py-12">
                        ...
                        <Link to="/transacoes/nova"
                            className="w-fit mx-auto mt-6 bg-primary-500 text-[#051626] px-4 py-2.5 rounded-xl flex items-center justify-center hover:bg-primary-600 transition-all">
                            <Plus className="w-4 h-4 mr-2" />
                            Nova Transação
                    ...
                    <div className="overflow-x-auto">
                        <table className="divide-y divide-gray-700 min-h-full w-full ">
                            <thead>
                                <tr>
                                    <th scope="col" className="px-3 py-3 text-left font-medium text-gray-400 text-xs uppercase">
                                        Descrição
                                    </th>
                                    <th scope="col" className="px-3 py-3 text-left font-medium text-gray-400 text-xs uppercase">
                                        Data
                            ...
                            <tbody className="divide-y divide-gray-700 w-full">
                                {filteredTransactions.map((transaction) => (
                                    <tr key={transaction.id} className="hover:bg-gray-800">
                                        ...
                                        <td className="px-3 py-4  whitespace-nowrap">
                                            <span className={`${transaction.type === TransactionType.INCOME ? 'text-primary-500'
                                                : 'text-red-600'
                                                }`}>
                                        ...
                                        <td className="px-3 py-4  whitespace-nowrap cursor-poointer">
                                            <button
                                                type="button"
                                                onClick={() => confirmDelete(transaction.id)}
                                                className="text-red-600 hover:text-red-700 rounded-full cursor-pointer"
                                                disabled={deletingId === transaction.id}
                                            >
                                                {deletingId === transaction.id ? (
                                                    <span className="inline-block w-4 h-4 border-2 border-red-700 border-t-transparent rounded-full animate-spin"
                                                    ></span>
                                                ) : (
